...
- Real-device bug-fix remediation (2026-07-23) independently verified and completed a previously interrupted, uncommitted diff that fixed 3 bugs the maintainer found testing the live site on a real phone (all automated tests had passed despite them): (1) `app.component.ts`'s `closeMenus()` now blurs `document.activeElement` when it is inside `.nav-shell`, releasing the `.nav-dropdown:focus-within` CSS fallback that kept the desktop mega-menu and mobile menu visually open after SPA navigation — confirmed via fresh Playwright runs that `aria-expanded`/`.open`/`:focus-within` all flip to closed after clicking a menu item, both mobile (390x844) and desktop (1440x900), with before/after screenshots; (2) `standard-page.component.ts` and `treatment-page.component.ts` TOC links now use `[routerLink]="[]" [fragment]="..."` instead of plain `href="#..."`, so clicking a "En esta página" link no longer risks a full navigation to home — confirmed by clicking every TOC link on a treatment page and a standard page (`/sobre-mi`) and asserting the resulting URL fragment plus that the target section's bounding rect is actually within the viewport; (3) `styles.scss` added `text-size-adjust: 100%` (prevents mobile browsers' automatic font-boosting) and tightened the 720px-breakpoint h1 clamp from `clamp(2.65rem, 15vw, 4.25rem)` to `clamp(2.5rem, 8vw, 4rem)` — verified as a real, measurable reduction (computed h1 font-size dropped from 48-59px to a flat 40px across iPhone SE/iPhone 12/Pixel 5 device-emulation profiles, confirmed via `git stash`-based before/after A-B testing, not just reading the diff's own comments) and confirmed `document.documentElement.scrollWidth - window.innerWidth === 0` at all 5 required viewports (360x800, 375x667, 390x844, 768x1024, 1440x900) x 4 required pages (`/`, sector hub, treatment page, `/contacto`) both before and after the fix (headless Chromium does not reproduce real-device font-boosting overflow, so this specific metric was already 0 pre-fix; the h1 size reduction and visual full-page screenshot review at all 20 combos are the actual evidence this round adds). No additional overflow-causing elements were found in the visual screenshot review; no code changes beyond the already-present diff were required. All 6 quality gates (`lint`, `typecheck`, `test`, `build`, `e2e`, `a11y`) were re-run fresh and passed; see the apply-progress evidence table. Tasks 6.2-6.4 remain intentionally blocked.
